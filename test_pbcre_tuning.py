...
from pbcre.tuning.tuning_grid import PBCREHyperparameterSearch
import pandas as pd
import numpy as np
...
if __name__ == "__main__":
    ...
    param_grid = {
        ...
        "criterion": ["hybrid"]  # puedes añadir gini, twoing
    }

    print("Buscando mejores hiperparámetros...")

    search = PBCREHyperparameterSearch(
    ...
    search.fit(
        proccessdata.getXtrain(),
        proccessdata.getytrain()
    )
    ...
    tree.save_cips_json(strong_cips, proccessdata.getytrain(), "cips_strong.json")
    tree.save_cips_csv(strong_cips, proccessdata.getytrain(), "cips_strong.csv")
    explanations = tree.explain_instances_with_cips(
        proccessdata.getXtest().values,
        strong_cips
    )

    tree.save_instance_explanations_json(explanations, "instance_explanations.json")
